...
>
> ```prompt
> `.github/prompts` 디렉터리에 `update-octofit-tracker-app.prompt.md`라는 프롬프트 파일을 생성하고, 해당 프롬프트 파일에 `mode: 'agent`와 `model: GPT-5.2`을 추가합니다.
> ```
...
> ```prompt
> users, teams, activities, leaderboard, and workouts 컬렉션을 지원하도록 `models.py`, `serializers.py`, `urls.py`, `views.py`, `tests.py`, `admin.py`를 업데이트합니다.
> ```
